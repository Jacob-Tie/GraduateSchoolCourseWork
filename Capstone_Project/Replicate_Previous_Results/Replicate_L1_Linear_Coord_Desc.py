...
#-------------------
#Subset data so it is easier to test. This is a required step for only this section since scikit learn uses 64 bit floats in its models, so we run into problems with memory:
#-------------------
num_samples = 100000
relData = relData[0:num_samples,:]
target = target[0:num_samples]
age = age[0:num_samples]
sex = sex[0:num_samples]
relData = relData.astype('float64')
saved_target = saved_target[0:num_samples]

#--------------------
#Replace missing values with avg, though it doesn't appear that any data is missing from the section of the UKBiobank data that we use
#--------------------
missing_count = 0
means = np.mean(relData, axis = 0)
for i in range(relData.shape[0]):
    for j in range(relData.shape[1]):
        if relData[i,j] < 0 or relData[i,j] > 2:
            missing_count += 1
            relData[i,j] = means[j]
f = open("./output.txt", "a")
f.write("Number of missing values in the train set: "+str(missing_count)+"\n")
f.close()

#same for the test set:
for i in range(relData_test.shape[0]):
    for j in range(relData_test.shape[1]):
        if relData_test[i,j] < 0 or relData_test[i,j] > 2:
            relData_test[i,j] = means[j]


#we will need to normalize reldata
from sklearn.preprocessing import Normalizer
transformer = Normalizer().fit(relData)

#Now we can train the model:
#Suppress the many warnings of scikit learn
...
#NOTE: This lambda overfits, but attains correlation close to what the paper gets on the train set.
#It is possible to use other lambdas to get better results on the test set, but given that we are unable to use the whole training set for this test we thought that it was better to show what the method
#is capable of rather than exactly reproducing results
lambdas = [.00000085]
#Keep track of progress
f = open("./output.txt", "a")
f.write("3\n")
...
#-------------
#A useful transformation: back to centimeters: reverses the various normalizations applied to the target variables, see above
#-------------
...
#--------------
#Normalize data
relData = transformer.transform(relData)
relData_test = transformer.transform(relData_test)

#Use lasso penalized regression as the model. This implementation of lasso uses coordinate descent, and requires the whole training set to be loaded into memory
clf = linear_model.Lasso(alpha=lambdas[0], random_state = 0)
clf.fit(relData, target)
